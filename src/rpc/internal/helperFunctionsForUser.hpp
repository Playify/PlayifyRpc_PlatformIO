namespace RpcInternal{
	...
	template<typename T>
	auto
	make_callReceiver(T t)->typename std::enable_if<!std::is_constructible<CallReceiver,T>::value,CallReceiver>::type{
		auto func=make_function(t);
		return [func](const FunctionCallContext& ctx,DataInput data){
			if(!data.tryCall(func,ctx)){
				ctx.reject(RpcDataError("Arguments do not meet parameter types"));
			}
		};
	}
	...
	CallReceiver make_smartProperty(T& ref,const std::function<void()>& onChange=nullptr){
		return [&ref,onChange](const FunctionCallContext& ctx,DataInput data){
			...
			}else return ctx.reject(RpcDataError("Arguments do not meet parameter types"));
			ctx.resolve(ref);
		};
	}
